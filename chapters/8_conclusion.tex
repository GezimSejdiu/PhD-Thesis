...
\begin{itemize}
    ...
    In Chapter~\ref{chapter:dist_lod_stats} we have demonstrated that our approach can scale and process large amount of \gls{RDF} data as compared with centralized approach.
    Nevertheless, we plan to further improve time efficiency by persisting the data to an even higher extend more in memory and perform load balancing, which could further improve the performance.
    Moreover, as our implementation is purely batch processing, in which the data chunks are normally very large we plan to investigate additional techniques for lowering the network overhead and io footprint.
    In this regard, efficient compression (e.g. \gls{HDT}~\cite{HDTFernandez2013}) methods for lowering the data communication would be very relevant.
    Finally, as our main focus in on applying distributed techniques to \gls{RDF} data processing, we plan to port the existing solution for near real-time computation of \gls{RDF} dataset statistics.
    ...
    In this regard, efficient strategies as well as detailed cost function for query plan optimization have to be considered.
    In addition, we also plan to consider other data management operations i.e. additions, updates, deletions and materialization of the results need to be considered as well.
    One solution could be provisioning the delta lake solution.
...
Main contributions of this thesis have been integrated within the SANSA framework and are making an impact to the semantic web community and several semantic web applications in the big data era -- resulting in a SANSA framework and being used in many European research projects.
