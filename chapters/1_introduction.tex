...
%One of the most common applications dealing with such RDF data are: (i) an apriori statistical information, (ii) quality assessment, and (iii) querying and retrieving information.
...
Therefore, there is a need for an comprehensive framework which offer capabilities of exploring, validating and querying large amount of RDF data at scale.
...
In this thesis, we aims to exploit the existing communication, synchronisation and distribution techniques to optimise the performance of Distributed Processing of RDF Datasets when dealing with large amount of RDF data.

\section{Problem Definition and Challenges}
\label{sec:problem-definition-and-challenges}
Processing large-scale RDF datasets is considered as one of the most challenging tasks in the Semantic Web~\cite{Benjamins02sixchallenges}.
...
In this thesis we describe our approach with best practices, tools and guidelines and show how it can be used (with some real use cases) to deploy and perform large-scale RDF processing easily and efficiently.
...
Within the scope of the thesis, we introduce a novel scalable approach for RDF dataset statistics computation.
...
With the focus on the scalability, we derive quality metrics defined in~\cite{zaveri2015quality} and propose a scalable and efficient quality assessment framework that compute different quality metrics for large RDF datasets.
...
With the objective of answering this research question, we investigate different data storage representation and SPARQL query evaluation and propose two different approaches for scalable and efficient RDF data processing.
First, we introduce Sparklify: a scalable software component for efficient evaluation of SPARQL queries over distributed RDF datasets.
It uses SPARQL-to-SQL rewriter techniques for translating SPARQL queries into Spark executable code.
Second approach we investigated and developed with the scope of this thesis, is a scalable approach to evaluate SPARQL queries over distributed RDF datasets using a semantic-based partitioning.
Results obtained during the research question~\ref{rq:3} allow us to address the defined challange (cf. Section~\ref{sec:c3}). 
Both approaches are implemented and integrated inside the state-of-the-art RDF processing framework: SANSA.
...
\begin{enumerate}
    ...
    First, Sparklify: a scalable software component for efficient evaluation of SPARQL queries over distributed RDF datasets. It uses Sparqlify as a SPARQL-to-SQL rewriter for translating SPARQL queries into Spark executable code.
    ...
    An evaluation of the performance of semantic-based approach in processing large-scale RDF datasets is also presented. 
    ...
    %    real use cases exploiting large-scale RDF datasets}.
    
    We collaborated with many different stockholders and resesarch projects during the development of this thesis in order to solve the real-world scalable knowledge analysis and \gls{RDF} processing use cases.
    ...
    Alethio uses our approach for efficient reading and processing of such large-scale RDF data (transactions on Ethereum blockchain) in order to perform analytics e.g. finding top accounts, or typical behavior patterns of exchanges' deposit wallets and more.
    ...
    BDE uses the Mu Swarm Logger service\furl{https://github.com/big-data-europe/mu-swarm-logger-service} for detecting docker events and convert their representation to RDF. 
    ...
    In particular, one challenge is to extract patterns from POI sets to discover \gls{AOI}s.
...
First, Sparklify: a scalable software component for efficient evaluation of SPARQL queries over distributed RDF datasets. 
It uses a SPARQL-to-SQL rewriter techniques for translating SPARQL queries into Spark executable code.
Second approach we investigated and developed with the scope of this thesis, is a scalable approach to evaluate SPARQL queries over distributed RDF datasets using a semantic-based partition.
