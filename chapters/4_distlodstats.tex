...
% We first introduce the main challenges while designing a scalabele approach for computing statistics over RDF datasets.
...
\begin{figure}
\centering
\includegraphics[width=.45\columnwidth]{images/4_distlodstats/distlodstats-rdd-lineage.png}
...
%As noted, data movement can occur then there is an attempt to group distributed data together in order to perform some computations like counting the number of similar elements. 
...
The complexity of $map()$ and $filter()$ itself is linear with respect to the number of input triples. %considered to be linear.
%Their complexity depends on the function passed to them.
The overall complexity depends on the functions passed to them. 
%Apart from this, the scale number plays a big role in the complexity in general. 
...
The complexity of the $sortBy$ operation according to Spark\furl{https://github.com/apache/spark/blob/d5b1d5fc80153571c308130833d0c0774de62c92/core/src/main/scala/org/apache/spark/Partitioner.scala\#L101} is a sampled $O(n)$, which means only the unique sample keys $m$ (with $m \le n$) are sorted and lead to a complexity of $O(m*log(m))$ plus the ranges of key sets. 
...
\begin{figure*}
\centering
\includegraphics[width=1.0\textwidth]{images/4_distlodstats/distlodstats-system.png}
...
\cline{2-6}
\rule{0pt}{8pt}
\multirow{2}{*}{$\longrightarrow$} & \multicolumn{2}{c|}{\scriptsize{LODStats}} & \multicolumn{3}{c}{\scriptsize{DistLODStats}} \\
...
\midrule
\scriptsize{\textbf{LinkedGeoData}}& \scriptsize{n/a} & \scriptsize{n/a} & \scriptsize{36.65/0.13} & \scriptsize{\textbf{4.37}/0.15} & \scriptsize{7.4x}\\
\scriptsize{\textbf{M$_{DBpedia}^{en}$}} & \scriptsize{\textbf{24.63}/0.57} & \scriptsize{fail} & \scriptsize{25.34/0.11} & \scriptsize{\textbf{2.97}/0.08} & \scriptsize{7.6x} \\
\scriptsize{\textbf{M$_{DBpedia}^{de}$}} & \scriptsize{n/a} & \scriptsize{n/a} & \scriptsize{10.34/0.06} & \scriptsize{\textbf{1.2}/0.0} & \scriptsize{7.3x}\\
\scriptsize{\textbf{M$_{DBpedia}^{fr}$}} & \scriptsize{n/a} & \scriptsize{n/a} & \scriptsize{10.49/0.09} & \scriptsize{\textbf{1.27}/0.04} & \scriptsize{7.3x}\\
\bottomrule
\end{tabularx}
\end{table*}
...
For demonstrating the usage of the tool, we have deployed it on the comprehensive statistics catalogue LODStats\furl{http://lodstats.aksw.org/} which crawls RDF data from metadata portals such as CKAN dataset metadata registry. By doing this, it obtains a comprehensive picture of the current state of the Web of Data.
